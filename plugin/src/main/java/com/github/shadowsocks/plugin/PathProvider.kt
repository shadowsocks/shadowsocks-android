 ...
 */
class PathProvider internal constructor(
    baseUri: Uri,
    ...
    fun addPath(path: String, mode: Int = 0b110100100): PathProvider {
        ...
        if (trimmed.startsWith(basePath)) {
            cursor.newRow().add(PluginContract.COLUMN_PATH, trimmed).add(
                PluginContract.COLUMN_MODE,
                mode
            )
        }
        return this
    ...
    fun addAt(file: File, at: String = "", mode: Int = 0b110100100): PathProvider {
        if (basePath.startsWith(at)) if (file.isDirectory) file.listFiles().forEach {
            addTo(it, at, mode)
        } else addPath(at, mode)
        return this
    }
}
