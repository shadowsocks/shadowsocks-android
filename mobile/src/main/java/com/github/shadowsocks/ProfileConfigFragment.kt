...
class ProfileConfigFragment : PreferenceFragmentCompat(), Preference.OnPreferenceChangeListener, OnPreferenceDataStoreChangeListener {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<Preference>(Key.metered)!!.apply {
            if (Build.VERSION.SDK_INT >= 28) {
                isEnabled = serviceMode == Key.modeVpn
            } else {
                remove()
            }
        }
    ...
    private fun initPlugins() {
        ...
        plugin.entries = plugins.map {
            it.value.label
        }.toTypedArray()
        plugin.entryValues = plugins.map {
            it.value.id
        }.toTypedArray()
        plugin.entryIcons = plugins.map {
            it.value.icon
        }.toTypedArray()
        plugin.entryPackageNames = plugins.map {
    ...
    override fun onResume() {
        ...
        val fallbackProfile = DataStore.udpFallback?.let {
    ...
    override fun onPreferenceDataStoreChanged(store: PreferenceDataStore, key: String) {
        if (key != Key.proxyApps && findPreference<Preference>(key) != null) {
            DataStore.dirty = true
        }
    }
                ...
                if (intent.resolveActivity(requireContext().packageManager) == null) {
                    ...
                    startActivityForResult(
                        intent.putExtra(
                            ...
                            pluginConfiguration.selectedOptions.toString()
                        ),
