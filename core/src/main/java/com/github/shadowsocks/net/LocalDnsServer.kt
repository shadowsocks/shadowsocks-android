...
class LocalDnsServer(
    private val localResolver: suspend (String) -> Array<InetAddress>,
    ...
    companion object {
        ...
        private fun prepareDnsResponse(request: Message) =
            Message(request.header.id).apply {
                header.setFlag(Flags.QR.toInt()) // this is a response
                if (request.header.getFlag(Flags.RD.toInt())) {
                    header.setFlag(Flags.RD.toInt())
                }
                request.question?.also {
                    addRecord(it, Section.QUESTION)
                }
            }
    ...
    private fun handlePacket(channel: DatagramChannel) {
        ...
        launch {
            ...
            while (channel.send(reply, source) <= 0) monitor.wait(
                channel,
    ...
    private suspend fun resolve(packet: ByteBuffer): ByteBuffer {
        val request = try {
            ...
            // we cannot parse the message, do not attempt to handle it at all
            printLog(e)
        ...
        return supervisorScope {
            ...
            try {
                ...
                if (remoteDomainMatcher?.containsMatchIn(host) == true) {
                    return@supervisorScope remote.await()
                }
                val localResults =
                    try {
                ...
                if (localIpMatcher.isEmpty() || localIpMatcher.any { subnet ->
                    localResults.any(subnet::matches)
                }) {
                    remote.cancel()
                ...
                when (e) {
                    is TimeoutCancellationException -> {
                        Crashlytics.log(Log.WARN, TAG, "Remote resolving timed out")
                    }
    ...
    private suspend fun forward(packet: ByteBuffer): ByteBuffer {
        ...
        return if (tcp) {
            SocketChannel.open().use { channel ->
                ...
                while (!channel.finishConnect()) monitor.wait(
                    channel,
                    SelectionKey.OP_CONNECT
                )
                while (channel.write(wrapped) >= 0 && wrapped.hasRemaining()) monitor.wait(
                    channel,
                    SelectionKey.OP_WRITE
                )
