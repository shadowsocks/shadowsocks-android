...
export ARCH=`uname -m`
export ANDROID_NDK_HOME=$HOME/.android/android-ndk-r12
...
if [ ! -d "$ANDROID_NDK_HOME" ]; then
    ...
    pushd $HOME/.android
    wget -q http://dl.google.com/android/repository/android-ndk-r12-linux-${ARCH}.zip
    unzip -q android-ndk-r12-linux-${ARCH}.zip
    popd
fi
